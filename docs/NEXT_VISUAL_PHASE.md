...
The current `prototype` HEAD is `3232c64` (full SHA `3232c644ff34fd2dd339915ed98338485955320e`). The hosted `depth` URL remains reachable with HTTP 200 and a 201085-byte response; the exact 8-check portrait matrix and fresh captures are recorded in the tick 85 build log. This is runtime evidence only; no human still-frame visual verdict is claimed.
