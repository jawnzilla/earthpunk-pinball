...
The current `prototype` HEAD is `3d29c11`. The hosted `depth` URL remains reachable with HTTP 200; the last complete matrix is recorded in the build log at `7892f47`. This is runtime evidence only; no human still-frame visual verdict is claimed for tick 83.
